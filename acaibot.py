...
PRICE_1_MONTH = 80   # 1个月 80 TRX
PRICE_2_MONTH = 130  # 2个月 130 TRX
PRICE_3_MONTH = 220  # 3个月 220 TRX
...
async def handle_photo_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    approve_keyboard = [
        [
            InlineKeyboardButton("✅ 确认1个月(80)", callback_data=f"img_approve_{uid}_1"),
            InlineKeyboardButton("✅ 确认2个月(130)", callback_data=f"img_approve_{uid}_2")
        ],
        [
            InlineKeyboardButton("✅ 确认3个月(220)", callback_data=f"img_approve_{uid}_3"),
            InlineKeyboardButton("❌ 拒绝凭证", callback_data=f"img_reject_{uid}")
        ]
    ]
    ...
    notification_caption = (
        f"📸 <b>【收到买家转账成功图】</b>\n\n"
        f"👤 <b>买家用户：</b> {username}\n"
        f"🆔 <b>买家 UID：</b> <code>{uid}</code>\n"
        f"----------------------------------------\n"
        f"💡 <b>主人操作提示：</b> 请核对波场账单。核对无误后点击下方按钮，机器人将自动为您执行充值建群开通！"
    )
    ...
    for m_id in masters:
        try:
            ...
            logging.error(f"转发图片给主人失败 {m_id}: {str(e)}")
            
    await update.message.reply_text("📥 <b>收到您的转账成功截图凭证！</b>\n系统已将其提交给创始主人进行多重风控核对，请耐心等待 1-3 分钟，开通后机器人会第一时间私聊通知您！", parse_mode="HTML")
...
async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_type == "private":
        ...
        if len(text) >= 20: # 提交纯文本哈希等凭证
            ...
            await update.message.reply_text("✅ 文本信息已递交审核！客服核对无误后将立即为您开通特权。")
            return
